...
T2 =T1+T
T_final=T2+T/4
n_t = 401 #number of points from T1 to T2
print( 1/(2*size_cat**2*np.sqrt(np.pi*k1/k2))*1/k2)
#Wigner params
nbWignerPlot = 15
nbCols=4
...
fidelity_tab_C=[]
fidelity_tab_T=[]
#final_state_C=init_state_C
#final_state_T= 0.5 * qt.ket2dm(qt.coherent(Na, alpha)) + 0.5 * qt.ket2dm(qt.coherent(Na, -alpha))
final_state_C=init_state_C
final_state_T= init_state_T


for (ii, t) in enumerate(tlist):
    fidelity_tab_C.append(np.sqrt(qt.expect(res_CNOT.states[ii].ptrace(0), final_state_C)))
    fidelity_tab_T.append(np.sqrt(qt.expect(res_CNOT.states[ii].ptrace(1), final_state_T)))

ii=int(n_t*T2*1.1*1/T_final)
print(fidelity_tab_C[ii])
print(fidelity_tab_T[ii])
...
#
#
#### Investigation of the states rotating
#tlist = np.linspace(0,T_final, n_t)
#rot_states=[]
#for time in tlist :
#    alpha_current=alpha*np.exp(1j*delta*time)
#    state_rot=qt.coherent(Na,alpha_current)+ 1j* qt.coherent(Na, -alpha_current)
#    state_rot=state_rot/state_rot.norm()
#    state_tot=qt.tensor(qt.coherent(Na,alpha),C_y_beta)+qt.tensor(qt.coherent(Na,-alpha),state_rot)
#    state_tot=state_tot/state_tot.norm()
#    rot_states.append(state_tot.ptrace(1))
#
#th_Wigner_T = compute_Wigner([-4,4, 501], nbWignerPlot,nbCols, n_t,-1)
#th_Wigner_T.draw_Wigner(rot_states, title=r'TARGET theor // $\alpha, \beta=$ %.0f -Trunc=%.0f //$\kappa_2=$%.0f - $\kappa_1=$%.0f// $\Delta=\frac{\alpha^2 \kappa_2}{c}$, c=%.0f //Control is on : %r// '%(size_cat,trunc,k2,k1,prop,control_on))
##plt.savefig(path+'wigner_TARGET_size%.0f_trunc%.0f_k1_%.0f_coefspeed_%.1f_control_on%r.png'%(size_cat,trunc,k1,prop,control_on))
#
#fig, ax=plt.subplots()
...
#space_size=[-4,4,101]
#space_size = np.linspace(space_size[0],space_size[1],space_size[2])
#state=qt.coherent(Na,-alpha)+1j*qt.coherent(Na,alpha) +qt.coherent(Na,alpha)+1j*qt.coherent(Na,-alpha)
#state=state/state.norm()
#wig = qt.wigner(state, space_size,space_size, g=2)
#ax.pcolor(space_size, space_size, wig, cmap='bwr', vmin=[-2/np.pi, 2/np.pi])
#ax.set_aspect('equal')
##
